...
<script>
  ...
  @scope val uploadScope  // Custom scope annotation
...
**Note:** `@scope` annotation makes it clear this is special compiler behavior, not a regular function call.

**Important:** `@scope` cannot have an initializer:
```whitehall
@scope val uploadScope           // ✅ OK
@scope val uploadScope = 10      // ❌ ERROR: @scope generates value automatically
```
...
- Parse `@scope val myScope` declaration
- Enforce no initializer (error if `@scope val x = value`)
...
### For Custom Scopes:
- ✅ `@scope val myScope` - Chosen (clear it's compiler magic, consistent with @prop/@store)
- ❌ `scope()` - Looks like regular function (confusing)
- ❌ `Scope()` - Constructor style (unclear it's special)
...
3. Add custom scope syntax (`@scope val myScope`)
...
3. 📋 Implement custom scope syntax (`@scope val myScope`)
4. 📋 Enforce no initializer for `@scope` (compile error)
